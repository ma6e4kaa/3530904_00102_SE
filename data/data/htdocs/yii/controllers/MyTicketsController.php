...
class MyTicketsController extends Controller
{
    ...
    public function actionIndex($id)
    {
        ...
        $dataProvider = $searchModel->searchMy($id, $this->request->queryParams);
    ...
    public function actionCreateFeedback($id)
    {
        ...
        $feedback = Feedback::find()->where(['tour_date_id' => $ticket->tour_date_id])->andWhere(['guest_id' => $ticket->guest_id])->one();
        if ($feedback) {
            $model = $feedback;
            if ($this->request->isPost) {
            ...
            if ($this->request->isPost) {
                if ($model->load($this->request->post())) {
                    $model->tour_date_id = $ticket->tour_date_id;
                    $model->guest_id = $ticket->guest_id;
                    if ($model->save()) {
                        return $this->redirect(['view', 'id' => $id]);
                    };
                }
        ...
        return $this->render('create-feedback', [
            'model' => $model,
            'ticket' => $this->findModel($id),
        ]);
    }
...
//                            проверка кол-ва товаров в наличии
    ...
    protected function findFeedbackModel($id)
    {
